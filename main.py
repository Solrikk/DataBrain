...
def transform_data(df):
    for column in df.columns:
        df[column] = df[column].apply(lambda x: str(x).capitalize() if pd.notnull(x) else x)
    return df

def extend_product_name(df):
    if 'Цвет' in df.columns and 'Наименование' in df.columns:
        additional_columns = ['Тип товара', 'Размеры (см)', 'Бренд', 'Коллекция', 'Артикул']

        def combine_columns(row):
            name = str(row['Наименование'])
            additions = [str(row[col]) for col in additional_columns if col in df.columns and str(row[col]).lower() != 'nan']
            combined = f"{name} {' '.join(additions)}"
            return ' '.join(sorted(set(combined.split()), key=combined.split().index))
...
def map_sections_by_similarity(df, section_mapping_filename='Разделы.xlsx'):
    ...
    if 'Разделы' in df.columns:
        ...
        df['Разделы'] = [section_df['Название'].iloc[np.argmax(row)] if max(row) > 0 else 'Без категории' for row in cosine_similarities]
    return df
...
def convert_to_template(filename, template_filename='Шаблон1.xlsx'):
    try:
        ...
        raise HTTPException(status_code=400, detail="The uploaded file is not a valid Excel file.")
...
async def upload_file(file: UploadFile = File(...)):
    ...
    try:
        ...
        return HTMLResponse(content=f"<html><body><h2>Error: {http_exc.detail}</h2></body></html>", status_code=http_exc.status_code)
